...
import { FALLBACK_GRADE } from "@/lib/fallback-data";
import { extractJsonBlock } from "@/lib/json-utils";
import type { GradeExerciseResult } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  const exercisePrompt: unknown = body?.exercisePrompt;
  ...
  const exercisePromptError = requireNonEmptyString(
    exercisePrompt,
    "exercisePrompt",
    MAX_TEXT_LENGTH
  );
  if (exercisePromptError) return exercisePromptError;

  const userAnswerError = requireNonEmptyString(userAnswer, "userAnswer", MAX_TEXT_LENGTH);
  if (userAnswerError) return userAnswerError;
  ...
  try {
    ...
    const generation = await model.generateContent(
      buildGradeExercisePrompt(skillName as string, exercisePrompt as string, userAnswer as string)
    );
    ...
    const result: GradeExerciseResult = {
      score: parsed.score,
    ...
    console.error("grade-exercise failed, using fallback:", error);
    return NextResponse.json(FALLBACK_GRADE);
  }
}
