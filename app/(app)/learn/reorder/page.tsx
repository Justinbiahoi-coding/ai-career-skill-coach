...
import type { DrillCardResult, ReorderStep } from "@/lib/types";
 ...
 */
export default function ReorderCardPage() {
  const router = useRouter();
  const [userEmail, setUserEmail] = useState<string | null | undefined>(undefined);
  const [context, setContext] = useState<Context | null>(null);
  const [checkedStorage, setCheckedStorage] = useState(false);

  const [items, setItems] = useState<ReorderStep[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-practice", {
      ...
      .then((data: DrillCardResult) => {
        if (cancelled) return;
        setItems(data.items as ReorderStep[]);
  ...
  useEffect(() => {
    if (allDone && context && !cardMarkedDone) {
      ...
      markCardCompletedForSkill(context.skillName, "reorder");
    }
