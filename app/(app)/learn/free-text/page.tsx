...
import type { DrillCardResult, FreeTextStep } from "@/lib/types";
...
export default function FreeTextCardPage() {
  const router = useRouter();
  const [userEmail, setUserEmail] = useState<string | null | undefined>(undefined);
  const [context, setContext] = useState<Context | null>(null);
  const [checkedStorage, setCheckedStorage] = useState(false);

  const [items, setItems] = useState<FreeTextStep[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-practice", {
      ...
      body: JSON.stringify({ skillName: context.skillName, jdText: context.jdText, mode: "free_text" }),
    })
      .then(async (res) => {
        if (!res.ok) {
      ...
      .then((data: DrillCardResult) => {
        if (cancelled) return;
        setItems(data.items as FreeTextStep[]);
  ...
  useEffect(() => {
    if (allDone && context && !cardMarkedDone) {
      ...
      markCardCompletedForSkill(context.skillName, "free_text");
    }
