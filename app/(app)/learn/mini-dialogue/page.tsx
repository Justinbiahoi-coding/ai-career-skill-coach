...
import type { DrillCardResult, MiniDialogueStep } from "@/lib/types";
...
export default function MiniDialogueCardPage() {
  const router = useRouter();
  const [userEmail, setUserEmail] = useState<string | null | undefined>(undefined);
  const [context, setContext] = useState<Context | null>(null);
  const [checkedStorage, setCheckedStorage] = useState(false);

  const [items, setItems] = useState<MiniDialogueStep[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-practice", {
      ...
      body: JSON.stringify({ skillName: context.skillName, jdText: context.jdText, mode: "mini_dialogue" }),
    })
      .then(async (res) => {
        if (!res.ok) {
      ...
      .then((data: DrillCardResult) => {
        if (cancelled) return;
        setItems(data.items as MiniDialogueStep[]);
  ...
  useEffect(() => {
    if (allDone && context && !cardMarkedDone) {
      ...
      markCardCompletedForSkill(context.skillName, "mini_dialogue");
    }
          ...
          <div className="flex flex-col gap-4 rounded-[30px] border border-carbon bg-paper-white p-6 sm:p-7">
            <div className="flex items-center justify-between gap-3">
              <span className="font-aeonik text-base font-extrabold text-carbon">
                Exchange {currentIndex + 1} of {items.length}
              </span>
              {isCurrentDone && (
            ...
            <MiniDialogueStepView
              key={currentIndex}
              openingQuestion={currentItem.openingQuestion}
              skillName={context.skillName}
              jdText={context.jdText}
              onComplete={() => handleComplete(currentIndex)}
            />
        ...
        {allDone && (
          <div className="flex flex-col items-center gap-3 rounded-[30px] border border-carbon bg-mint-pop/30 py-10 text-center">
            ...
            <p className="font-aeonik font-extrabold text-carbon">
              Card complete — all {items?.length} exchanges done.
            </p>
          </div>
        )}
