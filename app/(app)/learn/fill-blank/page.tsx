...
import type { DrillCardResult, FillBlankStep } from "@/lib/types";
 ...
 */
export default function FillBlankCardPage() {
  const router = useRouter();
  const [userEmail, setUserEmail] = useState<string | null | undefined>(undefined);
  const [context, setContext] = useState<Context | null>(null);
  const [checkedStorage, setCheckedStorage] = useState(false);

  const [items, setItems] = useState<FillBlankStep[] | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/generate-practice", {
      ...
      .then((data: DrillCardResult) => {
        if (cancelled) return;
        setItems(data.items as FillBlankStep[]);
  ...
  useEffect(() => {
    if (allDone && context && !cardMarkedDone) {
      ...
      markCardCompletedForSkill(context.skillName, "fill_blank");
    }
